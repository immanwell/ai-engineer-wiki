...
sources:
  - "The Expanding Toolkit.md"
status: stable
confidence: high
---
Created: Sunday, 10 May 2026, 18:59
...
**Event**: Code with Claude 2026 — Session 4
**Date**: May 6, 2026
**Watch**: https://www.youtube.com/live/GMIWm5y90xA

**Summary**: Scaffolding that used to be built manually now ships with the model. Covers four capability areas — tool use, context management, code execution, and computer use — unified by one rule: stop building compensating code for model unreliability; invest in connecting the model to your world.

**Sources**: [[raw/The Expanding Toolkit.md|The Expanding Toolkit — Code with Claude 2026]]
...
| Type of code | Half-life | Who should own it |
|---|---|---|
| **Compensating for model unreliability** (retries, routers, planners, verification loops) | Months — gets absorbed into the model | Anthropic |
| **Connecting your model to your world** (custom tools, your data, your specific context) | Compounding — model can't absorb what it can't see | You |

Every dollar spent on retry logic is a dollar that becomes obsolete when Anthropic ships the fix. Every dollar spent on your tool infrastructure compounds indefinitely.
...
- Layer retry decorators on top because tools failed often
- Routers = "guesses about user intent in conditional if statements" — first thing to break when adding new tools
...
- When a tool errors, Claude **sees the error, recovers on its own, and calls the tool again** — no retry routers needed

### Quick Tip: Output Schema in Tool Description

When giving a tool to Claude, describe the **output schema** — not just input parameters:
...
By describing the output, Claude knows what to expect. If `score` is returned, it can rank outputs without an extra round trip.

### Claude Code: Pre/Post Tool Hooks
...
### Quick Tip: Prune Stale Tool Results Every Turn
...
- [[agentic-architecture]] — agentic loop, tool use, computer use
- [[tool-design-mcp]] — tool design, output schema tip
- [[context-reliability]] — token management, prune stale results, 1M context
- [[claude-code]] — hooks, /schedule, slash context, Chrome extension
- [[advisory-strategy]] — Sonnet+Opus pattern embodies the "connecting code" investment thesis