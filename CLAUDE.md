...
- Always specify a language tag on the opening "```" of code blocks in /wiki/ files — use "```bash" for CLI commands, "```txt" for plain text/formulas, "```js" for JavaScript, "```python" for Python, "```json" for JSON, etc. Every code block must have a language tag; if it's not code, use "```txt". Only the opening "```" gets the tag, the closing "```" stays as plain "```".
